...
static int rtsp_msg_parse_auth (rtsp_msg_s *msg, const char *line)
{
	...
	if (0 == strncmp(p, "Basic ", 6)) {
		...
		char *ptr = strstr(p, "uri=\"");
		if (ptr == NULL) {
			fprintf(stderr, "Error: 1.uri=\" not found!\n");
			return -1;
		}
		
		char *ptr_uri_start = ptr+strlen("uri=\"");
		char *ptr_uri_end = strstr(ptr_uri_start, "\"");
		if (NULL == ptr_uri_end){
			fprintf(stderr, "Error: 2.\" not found!\n");
			return -1;
		}

		strncpy((void *)hdrs->auth->uri, (const void*)(ptr_uri_start), (ptr_uri_end-ptr_uri_start));
        hdrs->auth->uri[ptr_uri_end-ptr_uri_start] = '\0';

		ptr = strstr(p, "response=\"");
		if (ptr == NULL) {
			fprintf(stderr, "Error: 3.response=\" not found!\n");
			return -1;
		}
		
		char *ptr_response_start = ptr+strlen("response=\"");
		char *ptr_response_end = strstr(ptr_response_start, "\"");
		if (NULL == ptr_response_end){
			fprintf(stderr, "Error: 4.\" not found!\n");
			return -1;
		}

		strncpy((void *)hdrs->auth->digest_md5, (const void*)(ptr_response_start), (ptr_response_end-ptr_response_start));
        hdrs->auth->digest_md5[ptr_response_end-ptr_response_start] = '\0';

		printf("uri:%s,response:%s\n", hdrs->auth->uri, hdrs->auth->digest_md5);
	} else {
		rtsp_mem_free(hdrs->auth);
		hdrs->auth = NULL;
		err("parse %s failed. line: %s\n", "rtsp_msg_auth_s", line);
...
static int rtsp_msg_build_www_auth (const rtsp_msg_s *msg, char *line, int size)
{
	if (msg->hdrs.www_auth) {
		if (msg->hdrs.www_auth->type == RTSP_MSG_AUTH_TYPE_BASIC) {
			...
			return strlen(line);
		} if (msg->hdrs.www_auth->type == RTSP_MSG_AUTH_TYPE_DIGEST) {
			snprintf(line, size, "WWW-Authenticate: Digest realm=\"%s\", nonce=\"%s\"\r\n", 
					msg->hdrs.www_auth->realm, msg->hdrs.www_auth->nonce);
			return strlen(line);
		}else{
			return 0;
		}
...
int rtsp_msg_get_auth_digest (const rtsp_msg_s *msg, char *digest_md5, int len)
{
	if (!msg->hdrs.auth || msg->hdrs.auth->type != RTSP_MSG_AUTH_TYPE_DIGEST)
		return -1;
	if (digest_md5)
		strncpy(digest_md5, msg->hdrs.auth->digest_md5, len);
	return 0;
}

int rtsp_msg_set_www_auth_digest (rtsp_msg_s *msg, const char *realm, const char *nonce)
{
	if (!msg->hdrs.www_auth)
		msg->hdrs.www_auth = (rtsp_msg_www_auth_s*) rtsp_mem_alloc(sizeof(rtsp_msg_www_auth_s));
	if (!msg->hdrs.www_auth)
		return -1;
	msg->hdrs.www_auth->type = RTSP_MSG_AUTH_TYPE_DIGEST;
	strncpy(msg->hdrs.www_auth->realm, realm, 
			sizeof(msg->hdrs.www_auth->realm) - 1);
	strncpy(msg->hdrs.www_auth->nonce, nonce, 
			sizeof(msg->hdrs.www_auth->nonce) - 1);
	return 0;
}
