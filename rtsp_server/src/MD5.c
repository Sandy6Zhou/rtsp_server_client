...
const void md5_hash_to_hex_str(const uint8_t hash_array[], char output_string[]) {
...
void calculate_md5(char *data, char *md5_string, int len)
{
  if(len < 33){
    return;
  }
  ...
  strncpy((char*) hash_input, data, MD5_CHUNK_BYTES * chunk_count);
	...
	// printf("%s\n", hash_result_string);
  strncpy(md5_string, hash_result_string, sizeof(hash_result_string));
