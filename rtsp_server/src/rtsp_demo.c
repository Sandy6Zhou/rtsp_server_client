...
int rtsp_set_auth (rtsp_session_handle session, int type, const char *user, const char *passwd)
{
	...
	switch(type) {
		...
		case RTSP_AUTH_TYPE_DIGEST:
			...
			s->auth_type = RTSP_AUTH_TYPE_DIGEST;
			strcpy(s->auth_user, user);
			strcpy(s->auth_passwd, passwd);
			generate_nonce(s->nonce_b64, sizeof(s->nonce_b64));
		default:
...
//	struct rtsp_demo *d = cc->demo;
	...
	switch (s->auth_type) {
		...
		case RTSP_AUTH_TYPE_DIGEST:{
			if (rtsp_msg_get_auth_digest(reqmsg, digest_md5a, sizeof(digest_md5a))) {
				dbg("authorization get fail. [peer %s:%u]\n", inet_ntoa(cc->peer_addr), cc->peer_port);
				rtsp_msg_set_www_auth_digest(resmsg, "rtsp_demo", s->nonce_b64);
				rtsp_msg_set_response(resmsg, 401);//Unauthorized
				return 1;
			}
			
			char digest_up_1[256] = "";
			char digest_md5_1[33] = "";
			char digest_up_2[256] = "";
			char digest_md5_2[33] = "";
			char digest_up_3[1024] = "";

			sprintf(digest_up_1, "%s:%s:%s", s->auth_user, "rtsp_demo", s->auth_passwd);
			calculate_md5(digest_up_1, digest_md5_1, sizeof(digest_md5_1));

			char method[16]={0};
			if(reqmsg->hdrs.startline.reqline.method == RTSP_MSG_METHOD_DESCRIBE){
				strcpy(method, "DESCRIBE");
			}else if(reqmsg->hdrs.startline.reqline.method == RTSP_MSG_METHOD_SETUP){
				strcpy(method, "SETUP");
			}else if(reqmsg->hdrs.startline.reqline.method == RTSP_MSG_METHOD_PLAY){
				strcpy(method, "PLAY");
			}else if(reqmsg->hdrs.startline.reqline.method == RTSP_MSG_METHOD_PAUSE){
				strcpy(method, "PAUSE");
			}else if(reqmsg->hdrs.startline.reqline.method == RTSP_MSG_METHOD_TEARDOWN){
				strcpy(method, "TEARDOWN");
			}else{
				return -1;//TODO
			}

			sprintf(digest_up_2, "%s:%s", method, reqmsg->hdrs.auth->uri);
			calculate_md5(digest_up_2, digest_md5_2, sizeof(digest_md5_2));

			sprintf(digest_up_3, "%s:%s:%s", digest_md5_1, s->nonce_b64, digest_md5_2);
			calculate_md5(digest_up_3, digest_md5b, sizeof(digest_md5b));

			printf("digest_up_1:%s\ns->nonce_b64:%s\ndigest_up_2:%s\ndigest_up_3:%s\n", digest_up_1, s->nonce_b64, digest_up_2, digest_up_3);
			printf("digest_md5a:%s\ndigest_md5b:%s\n",digest_md5a,digest_md5b);
			
			if (strcmp(digest_md5a, digest_md5b)) {
				warn("authorization basic fail, my %s his %s. [peer %s:%u]\n", 
						basic_b64b, basic_b64a, inet_ntoa(cc->peer_addr), cc->peer_port);
				rtsp_msg_set_www_auth_digest(resmsg, "rtsp_demo", s->nonce_b64);
				rtsp_msg_set_response(resmsg, 401);//Unauthorized
...
//	struct rtsp_demo *d = cc->demo;
	...
	snprintf(apath, sizeof(apath) - 1, "%s/%s", s->path, ARTSP_SUBPATH);
...
static int rtsp_send_interlaced_frame (struct rtsp_client_connection *cc, int channel, const uint8_t *data, int len)
{
	...
	if (len + 4 > (int)sizeof(cc->resbuf)) {
		err("interlaced frame too big %d bytes, more then sizeof resbuf %ld bytes. "
...
static int rtsp_try_tx_rtcp_sr (struct rtsp_client_connection *cc, int isaudio, uint64_t ts)
{
	...
	ntpts_lsw = (uint32_t)(((ntptime_of_zero_ts + ts) % 1000000ULL) * (1ULL << 32) / 1000000ULL);//TODO
...
int is_send_data(rtsp_demo_handle demo)
{
	struct rtsp_demo *d = (struct rtsp_demo*)demo;
	
	if(d->connections_qhead.tqh_first){
		if(d->connections_qhead.tqh_first->state == RTSP_CC_STATE_PLAYING){
			return 1;
		}
	}
	return 0;
