...
const void _generate_nonce(char *nonce, int len)
{
    time_t current_time = time(NULL);
    unsigned int random_number = rand();
    snprintf(nonce, 256, "%lx%d", current_time, random_number);  // 时间戳 + 随机数
}

void generate_nonce(char *nonce_b64, int len)
{
	char nonce[256];
	_generate_nonce(nonce, sizeof(nonce));
	base64_encode(nonce_b64, len, (uint8_t*)nonce, strlen(nonce));
}
