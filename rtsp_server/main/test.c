...
#define DEMO_CFG_FILE "../etc/demo.cfg"
...
int main(int argc, char *argv[])
{
	...
	for (ch = 0; ch < session_count; ch++) {
		...
		rtsp_set_auth(session[ch], RTSP_AUTH_TYPE_DIGEST, "admin", "123456");
	...
	while (flag_run) {
		...
		for (ch = 0; ch < session_count; ch++) {
			if (fp[ch][0]) {
				if(is_send_data(demo)==0){continue;}//有客户端连接才开始发送数据
			...
			read_video_again:
				...
				if (session[ch])
					rtsp_tx_video(session[ch], vbuf, vsize, ts);//vbuf一帧视频帧(start code+nalu header+payload)
