...
#include "md5.h"
static CmdTbl gcmdtbl[]={{"OPTIONS", 1},
                        {"DESCRIBE", 2},
                        {"SETUP", 4},
                        {"PLAY", 8},
                        {"PAUSE", 16},
...
void GetSdpVideoAcontrol(char *buf, uint32_t size, RtspSession *sess)
{
    char *ptr = (char *)memmem((const void*)buf, size,
            (const void*)SDP_M_VIDEO, strlen(SDP_M_VIDEO));
    if (NULL == ptr){
        fprintf(stderr, "Error: m=video not found!\n");
        return;
    }

    ptr = (char *)memmem((const void*)ptr, size-(ptr-buf),
            (const void*)SDP_A_CONTROL, strlen(SDP_A_CONTROL));
    if (NULL == ptr){
        fprintf(stderr, "Error: a=control not found!\n");
        return;
    }

    char *endptr = (char *)memmem((const void*)ptr, size-(ptr-buf),
            (const void*)"\r\n", strlen("\r\n"));
    if (NULL == endptr){
...
void GetSdpVideoTransport(char *buf, uint32_t size, RtspSession *sess)
{
    ...
    ptr = (char *)memmem((const void*)ptr, size,
            (const void*)UDP_TRANSPORT, strlen(UDP_TRANSPORT));
    if (NULL != ptr){
        sess->trans = RTP_AVP_UDP;
    }else{
        ptr = (char *)memmem((const void*)ptr, size,
                (const void*)TCP_TRANSPORT, strlen(TCP_TRANSPORT));
        if (NULL != ptr)
...
int32_t ParseUnauthorizedMess(char *buf, uint32_t size, RtspSession *sess)
{
    ...
    if (NULL == p) {
        printf("%s not found\n",AUTH_NONCE);
        sess->auth_struct.auth_mode = AUTH_BASIC;
        return False;
    }
    p += strlen(AUTH_NONCE);
    char *p_end = strstr(p+1,"\"");
    memset(sess->auth_struct.nonce,0x00,sizeof(sess->auth_struct.nonce));
    memcpy(sess->auth_struct.nonce,p+1,p_end-(p+1));
    free(tmp);
    sess->auth_struct.auth_mode = AUTH_DIGEST;
...
{
    uint16_t ha1_length,ha2_length,nonce_length,response_length;
    ...
    nonce_length = strlen(sess->auth_struct.nonce);
    response_length = 32*2 + nonce_length + 2;
    ...
    md5((uint8_t *)ha1_str,ha1_length,ha1);
    ...
    md5((uint8_t *)ha2_str,ha2_length,ha2);
   // memcpy(sess->auth_struct.ha2,ha2_str,sizeof(sess->auth_struct.ha2));
    ...
    md5((uint8_t *)response_str,response_length,sess->auth_struct.auth_response);
    free(ha1);
    free(ha2);
    printf("\n%s",response_str);
    printf("\nresponse_length %d\n",response_length);
